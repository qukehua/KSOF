...
class Options:
    ...
    def _initial(self):
        ...
        self.parser.add_argument('--point_npy', type=str, default='point_npy', help='path to save checkpoint.npy')
        self.parser.add_argument('--truth_npy', type=str, default='truth_npy', help='path to save truth_checkpoint.npy')
        self.parser.add_argument('--predict_npy', type=str, default='predict_npy',
                                 help='path to save predict_checkpoint.npy')

        # ===============================================================
        #                     Model options
        # ===============================================================
        self.parser.add_argument('--input_feature', type=int, default=10, help='size of pose Embedding layer')
        self.parser.add_argument('--cat_feature', type=int, default=132)
        self.parser.add_argument('--num_gcn', type=int, default=4, help='number of GCN ')
        self.parser.add_argument('--hidden_gcn', type=int, default=512, help='number of GCN hidden features')
        self.parser.add_argument('--mask_ratio', type=int, default=0.16, help='ratio of joints mask')
        self.parser.add_argument('--node_n', type=int, default=66, help='number of GCN nodes')
        self.parser.add_argument('--drop_out', type=float, default=0.75, help='drop out probability')
        self.parser.add_argument('--num_tcn', type=int, default=4)
        self.parser.add_argument('--num_num_channels', type=int, default=[30,30,30,30])
        self.parser.add_argument('--hidden_Spatial', type=int, default=128, help='number of SpatialMLP')
        self.parser.add_argument('--kernal_size', type=int, default=2)
        ...
        self.parser.add_argument('--output_n', type=int, default=10, help='future frame number') # 10为短时，25为长时
        self.parser.add_argument('--lr_now', type=float, default=0.0005)#学习率
        self.parser.add_argument('--max_norm', type=float, default=10000)
        self.parser.add_argument('--epoch', type=int, default=50)   #epoch数
        self.parser.add_argument('--batch_size', type=int, default=50)  # 16 for multistage model
        self.parser.add_argument('--test_batch_size', type=int, default=256)  # 16 for multistage model
        self.parser.add_argument('--is_load', dest='is_load', action='store_true',
                                 help='whether to load existing model')
        self.parser.add_argument('--test_sample_num', type=int, default=256, help='the num of sample, '                                                                          'that sampled from test dataset'
                                                                                  '{8,256,-1(all dataset)}')
