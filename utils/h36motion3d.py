...
class Datasets(Dataset):
    def __init__(self, opt, actions=None, split=0):
        ...
        subs = np.array([[1, 6, 7, 8, 9], [5], [11]], dtype=object)  # changed
            ...
            for action_idx in np.arange(len(acts)):
                ...
                if self.split <= 1 or opt.test_sample_num < 0:
                    for subact in [1, 2]:  # subactions
                        ...
                        the_sequence = torch.from_numpy(the_sequence).float().to(self.opt.cuda_idx)
                    ...
                    the_seq1 = torch.from_numpy(the_sequence1).float().to(self.opt.cuda_idx)
                    the_seq1[:, 0:6] = 0
                    ...
                    the_seq2 = torch.from_numpy(the_sequence2).float().to(self.opt.cuda_idx)
                    the_seq2[:, 0:6] = 0
