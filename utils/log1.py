...
def save_npy_log(opt, value, is_create=False, file_name='walking'):
    if not os.path.isdir(opt.point_npy): # 创建路径
        os.makedirs(opt.point_npy)

    if len(value.shape) < 2:
        value = np.expand_dims(value, axis=0)
    file_path = opt.point_npy + '/{}.npy'.format(file_name)
    print(file_path)
