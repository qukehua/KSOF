...
from utils.dpw3_opt import Options
...
def main(opt):
	...
	if not opt.is_eval:
		...
		valid_dataset = PW3_Motion3D.Datasets(opt, split=1)  # changed
		print('>>> Validation dataset length: {:d}'.format(valid_dataset.__len__()))
		valid_loader = DataLoader(valid_dataset, batch_size=opt.test_batch_size, shuffle=True, num_workers=0,
		                          pin_memory=True)

	test_dataset = PW3_Motion3D.Datasets(opt, split=1)  # changed
...
def eval(opt):
	...
	dataset = PW3_Motion3D.Datasets(opt=opt, split=1)  # changed
	dim_used = dataset.dim_used
...
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None, dim_used=None):
	...
	if is_train <= 1:
		...
		titles = (np.array(range(opt.output_n)) + 1) * 1000/30
	...
	for i, (p3d_h36) in tqdm(enumerate(data_loader), total=len(data_loader)):
		...
		p3d_sup_4 = p3d_h36.clone()[:, :, dim_used][:, -out_n:].reshape(
			[-1, out_n, len(dim_used) // 3, 3])
		...
		# p3d_out_all_4, p3d_out_all_3, p3d_out_all_2, p3d_out_all_1 = net_pred(input, input_n=in_n, output_n=out_n,
		#                                                                       itera=itera)
		p3d_out_all_4 = net_pred(input)

		p3d_out_4 = p3d_h36.clone()[:, in_n:in_n + out_n]
		p3d_out_4[:, :, dim_used] = p3d_out_all_4
		...
		p3d_out_all_4 = p3d_out_all_4.reshape([batch_size, out_n, len(dim_used) // 3, 3])
	...
	if is_train <= 1:
		...
		for j in range(out_n):
			ret["#{:d}ms".format(int(titles[j]))] = m_p3d_h36[j]
	return ret
